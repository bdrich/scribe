...
// ExecutePathTemplates reads a YAML/JSON file from the valuesIn stream, uses it as values
// to populate the tplPath path and values and writes the executed templates to
// the out stream.
...
// ExecuteTemplates -- Reads a YAML document from the values_in stream, uses it as values
